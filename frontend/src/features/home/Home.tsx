...
export function Home({
  ...
  return (
    <section className="home-content">
      ...
      {isLoadingSummary ? <section className="home-attention home-attention-skeleton" aria-label="Cargando pendientes"><div className="section-heading"><div><SkeletonLine className="skeleton-eyebrow" /><SkeletonLine className="skeleton-heading" /></div></div><SkeletonLine className="skeleton-attention-row" /><SkeletonLine className="skeleton-attention-row" /></section> : recurringInstances.length || pendingItems.length ? <details className="home-attention" open><summary className="section-heading"><div><p className="eyebrow">PARA RESOLVER</p><h2>Lo que requiere atención</h2></div><span>{recurringInstances.length + pendingItems.length} pendientes <ChevronDown className="home-attention-chevron" size={18} aria-hidden="true" /></span></summary><div className="home-attention-content">{recurringInstances.length ? <section className="recurring-due-widget">
        ...
        </section> : recurringError ? <ErrorState message={recurringError} onRetry={() => void loadRecurringInstances()} /> : null}{pendingItems.length ? <PendingWidget token={token} accounts={accounts} items={pendingItems} onChanged={onPendingChanged} /> : null}</div></details> : recurringError ? <ErrorState message={recurringError} onRetry={() => void loadRecurringInstances()} /> : null}
