...
vi.mock("../api/client", () => ({ corregirOcr: vi.fn() }));
...
beforeEach(() => corregirOcrMock.mockReset());
...
describe("PendingWidget", () => {
  ...
  it("submits corrections and notifies the parent", async () => {
    ...
    await waitFor(() => expect(corregirOcrMock).toHaveBeenCalledWith("token-123", "ocr-1", expect.objectContaining({ cuentaId: account.id, categoria: "OTROS" })));
